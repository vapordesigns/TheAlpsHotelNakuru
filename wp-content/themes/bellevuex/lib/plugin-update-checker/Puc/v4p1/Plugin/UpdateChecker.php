	...
	class Puc_v4p1_Plugin_UpdateChecker extends Puc_v4p1_UpdateChecker {
		...
		public function displayManualCheckResult() {
			if ( isset($_GET['puc_update_check_result'], $_GET['puc_slug']) && ($_GET['puc_slug'] == $this->slug) ) {
				...
				if ( $status == 'no_update' ) {
					$message = sprintf(_x('The %s plugin is up to date.', 'the plugin title', 'bellevue'), $title);
				} else if ( $status == 'update_available' ) {
					$message = sprintf(_x('A new version of the %s plugin is available.', 'the plugin title', 'bellevue'), $title);
				} else {
					$message = sprintf(__('Unknown update checker status "%s"', 'bellevue'), htmlentities($status));
				}
				printf(
