		...
		FusionPageBuilder.fusion_content_box = FusionPageBuilder.ChildElementView.extend( {
			...
			buildShortcodeIconWrapperAttr: function( atts ) {
				...
				if ( values.icon ) {
					...
					if ( 'yes' === parentValues.icon_circle ) {
						...
						} else if ( values.outercirclebordersize && 0 !== parseFloat( values.outercirclebordersize ) ) {
			...
			contentBoxShortcodeAttrs: function( readmore, extraClass, atts ) {
				...
				if ( ( 'button' === parentValues.link_type || 'button-bar' === parentValues.link_type ) && readmore ) {
					contentBoxShortcodeLink[ 'class' ] += 'fusion-read-more-button fusion-content-box-button fusion-button button-default button-' + extras.button_size.toLowerCase() + ' button-' + extras.button_shape.toLowerCase() + ' button-' + extras.button_type.toLowerCase();
				}
