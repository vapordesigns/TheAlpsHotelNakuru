	...
	jQuery( document ).ready( function() {
		...
		FusionPageBuilder.fusion_button = FusionPageBuilder.ElementView.extend( {
			...
			buildValues: function( values ) {
				...
				oldBorderColor = values.border_color;
				oldTextColor   = values.text_color;

				if ( '' !== oldBorderColor ) {
					values.border_color = oldBorderColor;
				}

				if ( '' !== oldTextColor ) {
					values.text_color = oldBorderColor;
				}
			...
			buildAttr: function( values ) {
				var params = this.model.get( 'params' ),
					...
					shapeClass   = 'fusion-button-' + values.shape,
					typeClass    = '';
				...
				if ( ( 'undefined' !== typeof params.shape && ( '' === params.shape || 'default' === params.shape ) ) || 'undefined' === typeof params.shape ) {
					shapeClass = 'fusion-button-default-shape';
				}
				...
				attr[ 'class' ] += ' ' + sizeClass + ' ' + stretchClass + ' ' + shapeClass + ' ' + typeClass;
			...
			buildButtonStyles: function( values ) {
				...
				if ( ( 'custom' === values.color || 'default' === values.color || ( -1 !== values.color.indexOf( 'scheme-' ) && ( '' !== values.bevel_color || '' !== values.accent_color || '' !== values.accent_hover_color || '' !== values.border_width || '' !== values.gradient_colors ) ) ) ) {
					...
					if ( 'default' !== values.color ) {
						...
						if ( '' !== oldBorderColor ) {
							generalStyles += 'border-color:' + oldBorderColor + ';';
						} else if ( values.accent_color ) {
							generalStyles += 'border-color:' + values.accent_color + ';';
						}
						...
						if ( '' !== oldBorderColor ) {
							hoverStyles += 'border-color:' + oldBorderColor + ';';
						} else if ( '' !== values.accent_hover_color ) {
							hoverStyles += 'border-color:' + values.accent_hover_color + ';';
						} else if ( '' !== values.accent_color ) {
