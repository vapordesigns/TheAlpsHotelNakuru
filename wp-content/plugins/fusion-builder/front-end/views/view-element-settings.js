...
( function() {
	...
	FusionPageBuilder.ElementSettingsView = window.wp.Backbone.View.extend( {
		...
		optionChange: function( event ) {
			...
			if ( ! jQuery( event.target ).hasClass( 'fusion-skip-debounce' ) && ( this.model.get( 'noTemplate' ) || jQuery( event.target ).hasClass( 'fusion-debounce-change' ) ) ) {
				_.fusionDebounce( this.optionChanged( event ), 500, paramName );
			} else {
		...
		debouncedInitOptions: function( $element ) {
			...
			if ( ( tabId && true === this.tabsRendered ) || ( 'undefined' !== typeof this.tabsRendered[ tabId ] && this.tabsRendered[ tabId ] ) || true === this.tabsRendered ) {
				return;
			}
