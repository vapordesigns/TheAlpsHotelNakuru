		...
		FusionPageBuilder.ContainerView = FusionPageBuilder.BaseView.extend( {
				...
				if ( true === videoBg ) {
					...
					if ( '' !== values.video_url ) {
						...
						if ( 'youtube' === videoUrl.type ) {
							outerHTML += '<div style=\'visibility: hidden\' id=\'video-' + cid + '\' data-youtube-video-id=\'' + videoUrl.id + '\' data-mute=\'' + values.video_mute + '\' data-loop=\'' + loop + '\' data-loop-adjustment=\'' + values.video_loop_refinement + '\' data-video-aspect-ratio=\'' + values.video_aspect_ratio + '\'><div id=\'video-' + cid + '-inner\'></div></div>';
					...
					// Video Overlay.
					if ( '' !== values.background_color && 1 > alpha ) {
			...
			onOptionChange: function( paramName, paramValue, event ) {
				...
				case 'background_color':
					if ( 0 === jQuery.Color( paramValue ).alpha() || 1 === jQuery.Color( paramValue ).alpha() ) {
						this.$el.find( '.fusion-fullwidth' ).removeClass( 'fusion-blend-mode' );
					} else {
						this.$el.find( '.fusion-fullwidth' ).addClass( 'fusion-blend-mode' );
					}

					if ( 1 > jQuery.Color( paramValue ).alpha() && this.$el.find( '.parallax-inner' ).length ) {
						this.$el.find( '.parallax-inner' ).css( 'background-color', paramValue );
					} else if ( 1 === jQuery.Color( paramValue ).alpha() && this.$el.find( '.parallax-inner' ).length ) {
						this.$el.find( '.parallax-inner' ).css( 'background-color', 'transparent' );
					}
			...
			patcherFilter: function( diff ) {
				...
				_.each( diff, function( info ) {
					if ( 'removeElement' === info.action ) {
						if ( 'undefined' !== typeof info.element.attributes && -1 !== info.element.attributes[ 'class' ].indexOf( 'fusion-container-spacing' ) ) {
